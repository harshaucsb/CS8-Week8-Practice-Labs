...
class BaseClass(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # TODO :  can rename this and add more in case we want to add more modules to be tested
        cls.student_function_filename = config.required_files[1][:-3]  # remember to remove the .py extension
        cls.student_functions = cls.import_module_util(cls.student_function_filename)
    ...
    def setUp(self) -> None:
        if type(self.student_functions) == str:
            self.fail(f"There is an error in {self.student_function_filename}.py:\n{self.student_functions}\n")
    ...
    def handle_test_function_logic(self, params, function_name, input_line, student_module):
        ...
        parameters_to_show = "(" + ', '.join(str(i) if type(i) != str else "'%s'" % i for i in params) + ")"
        output = []
        output.append(star_line)
        output.append(f"Input arguments:\n\n{parameters_to_show}\n")
        output.append("Checking for function logic...")
        is_correct = True
    ...
    def handle_test_print_return_value(self, params, student_function_name, input_line, student_module):
        '''
        ...
        '''
        ############### CORRECT PART #####################
        corr_params, corr_stdout, expected_answer, stud_stdout, student_answer = self.handle_test_util(
            student_function_name,
            input_line,
            params,
            student_module)
        parameters_to_show = "(" + ', '.join(str(i) if type(i) != str else "'%s'" % i for i in params) + ")"
        ...
        output.append(star_line)
        output.append(f"Input arguments: \n\n{parameters_to_show}\n")
        output.append("Checking for print statements and return type...")
    ...
    def handle_test_util(self, student_function_name, input_line, params, student_module):
        '''
        ...
        given function.
        '''
        corr_params, corr_stdout, expected_answer = self.get_expected_result(student_function_name, input_line, params)
        stud_stdout, student_answer = self.get_student_result(config.fun_names_dict[student_function_name], input_line,
                                                              params, student_module)
        ############### CHECK PART #####################
        # if student_answer is None:
        #     self.fail(f"Do you have a return statement in the right place?")
        return corr_params, corr_stdout, expected_answer, stud_stdout, student_answer

    def get_student_result(self, student_func_name, input_line, params, student_module):
    ...
    def get_expected_result(self, correct_function_name, input_line, params):
        correct_func = getattr(correct, correct_function_name)
        to_input = input_line
        # Save the previous area where print() will go, and create our own
        old_stdout = sys.stdout
